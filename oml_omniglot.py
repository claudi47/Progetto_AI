...
def main():
    ...
    args['traj_classes'] = list(range(int(100/2), 100)) # 963 for omniglot dataset
    ...
    # CIFAR-10 dataset
    resize = T.Resize((84, 84))
    to_tensor = T.ToTensor()
    greyscale = T.Grayscale()
    normalize = T.Normalize(mean=(0.4914,), std=(0.247,))

    train_transform = T.Compose([resize, greyscale, to_tensor, normalize])
    test_transform = T.Compose([resize, greyscale, to_tensor, normalize])

    # dataset = datasets.CIFAR10(root=".", download=True, train=True, transform=train_transform)
    # dataset_test = datasets.CIFAR10(root=".", download=True, train=False, transform=test_transform)

    dataset = datasets.CIFAR100(root=".", download=True, train=True, transform=train_transform)
    dataset_test = datasets.CIFAR100(root=".", download=True, train=False, transform=test_transform)

    # Iterators used for evaluation
    iterator_test = torch.utils.data.DataLoader(dataset_test, batch_size=64, # batch 64 for cifar
    ...
    config = mf.ModelFactory.get_model("na", args['dataset'], output_dimension=1000) # 10, 100 for cifar
